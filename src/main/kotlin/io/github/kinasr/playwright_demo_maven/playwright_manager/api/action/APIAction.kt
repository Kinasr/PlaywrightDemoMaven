...
import io.github.kinasr.playwright_demo_maven.playwright_manager.api.model.APIRequest
...
class APIAction(
    ...
    private fun <T> send(
        ...
        endpoint: String,
        bodyType: Class<T>,
        options: (APIRequest.() -> Unit) = {}
    ): APIResult<T> {
        APIRequest(APIMethod.GET)
        requestManager.use { manager ->
            val requestOptions = APIRequest(method).apply {
                options()
                this.maxRetries = this.maxRetries.ifNull { config.api.maxRetries }
            }

            fun executeRequest(): APIResult<T> {
                return manager.request.fetch(
                    endpoint,
                    requestOptions.toRequestOptions(jsonConverter)
                )
                    .result(bodyType, jsonConverter, validationBuilder)
            }

            val jsonRequest = requestOptions.toPrettyJsonString(jsonConverter)
            val logMessage = "Sending request to: ${method.str}: ${manager.baseURL}$endpoint"
            logger.info { logMessage }
            logger.apiDebug { "Request options: $jsonRequest" }
            ...
            return try {
                ...
                logger.apiDebug { "Response: $result" }
    ...
    fun get(endpoint: String, bodyType: Class<String>, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return send(APIMethod.GET, endpoint, bodyType, options)
    }

    fun get(endpoint: String, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return get(endpoint, String::class.java, options)
    }

    fun post(endpoint: String, bodyType: Class<String>, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return send(APIMethod.POST, endpoint, bodyType, options)
    }

    fun post(endpoint: String, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return post(endpoint, String::class.java, options)
    }

    fun put(endpoint: String, bodyType: Class<String>, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return send(APIMethod.PUT, endpoint, bodyType, options)
    }

    fun put(endpoint: String, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return put(endpoint, String::class.java, options)
    }

    fun delete(endpoint: String, bodyType: Class<String>, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return send(APIMethod.DELETE, endpoint, bodyType, options)
    }

    fun delete(endpoint: String, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return delete(endpoint, String::class.java, options)
    }

    fun patch(endpoint: String, bodyType: Class<String>, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return send(APIMethod.PATCH, endpoint, bodyType, options)
    }

    fun patch(endpoint: String, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return patch(endpoint, String::class.java, options)
    }

    fun head(endpoint: String, bodyType: Class<String>, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
        return send(APIMethod.HEAD, endpoint, bodyType, options)
    }

    fun head(endpoint: String, options: (APIRequest.() -> Unit) = {}): APIResult<String> {
